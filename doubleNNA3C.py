...
parser.add_argument("--nn_type", type=int, default=12)
...
# STAT_FILE = "experiments/part{}/nn{}_env{}_mask{}_Stat.csv".format(args.part, args.nn_type, args.env_type, args.mask)
WEIGHTS_FILE = "experiments/double/weights.h5".format(args.part, args.nn_type, args.env_type, args.mask)
...
class Brain:
	...
	def __init__(self, nn=12, mask=True, load_weights=False):
		input_layer, last_layer, placeholder, make_input, NONE_STATE = getNN(12, True);
		...
		self.modelOld = self._build_model(input_layer, last_layer)

		input_layer, last_layer, placeholder, _, _ = getNN(12, True);
		...
		self.load("experiments/part0/nn12_env1_maskTrue_weights.h5")
	...
	def load(self, name):
		self.modelOld.load_weights(name)
